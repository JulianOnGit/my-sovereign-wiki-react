...
import { getLLMConfig, setLLMConfig, clearLLMConfig, localLLMReady } from "../lib/llm.js";
...
const SOVEREIGN_AI_OPTIONS = [
  {
    key: "local",
    icon: "🖥️",
    name: "Local LLM server (Msty / Ollama / LM Studio)",
    tagline:
      "A real model on your own machine — picked up automatically by Organise, Wiki summaries and Ask. Endpoint & model are set in the Ask your LLM tab.",
    where: "Your device · localhost",
    status: "active",
  },
  {
    key: "none",
    icon: "🔒",
    name: "On-device heuristic engine",
    tagline: "Deterministic, dependency-free pass in your browser. Works with no model at all.",
    where: "Your device",
    status: "active",
  },
  {
...
function AiProviderCard() {
  ...
  function disconnect() {
    // Back to the zero-config default: auto (local model when running).
    clearLLMConfig();
    setProvider("auto");
    setKey("");
    setSaved(false);
  ...
  // Selecting an option: the two on-device engines need no key, the key options
  // set the provider, and the hosted-sovereign previews explain themselves rather
  // than pretend to connect.
  function choose(opt) {
    setSaved(false);
    if (opt.status === "preview") {
      setNote(
        `${opt.name} is on the sovereign-AI roadmap — a hosted option where the model runs ` +
          ...
          `the one source of truth. Today, use the on-device engines or your own key.`,
      );
      return;
    }
    setNote(
      opt.key === "local" && !localLLMReady()
        ? "No local model server answered just now — start Msty/Ollama (endpoint is set in the Ask your LLM tab) and the features pick it up automatically."
        : null,
    );
    setProvider(opt.key);
  }

  // "auto" (the zero-config default) highlights the local-LLM tile, since that's
  // what auto resolves to when no key is saved.
  const selectedKey = provider === "auto" ? "local" : provider;
  ...
  return (
    <div className="card">
      <div className="govern-card-head">
        ...
        <span
          className={`ai-status ${
            initial.provider === "anthropic" || initial.provider === "openai"
              ? "is-connected"
              : "is-local"
          }`}
        >
      ...
      <div className="ai-form">
        ...
        <div className="ai-actions">
          <button className="save" onClick={save} disabled={needsKey && !key.trim()}>
            {saved
              ? "Saved"
              : provider === "none"
